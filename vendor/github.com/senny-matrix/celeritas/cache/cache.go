...
func (c *RedisCache) Get(str string) (any, error) {
	...
	decodedEntry, err := decode(string(cacheEntry))
	if err != nil {
		return nil, err
	}

	item := decodedEntry[key]
...
func (c *RedisCache) Set(str string, value any, expires ...int) error {
	...
	if len(expires) > 0 {
		_, err = conn.Do("SETEX", key, expires[0], string(encode))
		if err != nil {
			return err
		}
	} else {
		_, err = conn.Do("SET", key, string(encode))
		if err != nil {
			return err
		}
	}
...
func (c *RedisCache) getKeys(pattern string) ([]string, error) {
	...
	for {
		arr, err := redis.Values(
			conn.Do(
				"SCAN", iter, "MATCH", fmt.Sprintf(
					"%s*", pattern)))
		if err != nil {
			return keys, err
		}
